...
int cpt_cannal=0, temp1=123, temp2=56, temp3=56, Vref1=0,Vref2=0,Vref3=0, Vref4=0;
...
float ratio_timer=4.23;

void setup() 
{
    //Timer1.initialize(10000); Timer1.attachInterrupt(PWM_Activation); Timer1.stop();
    Serial.begin(115600); Serial.setTimeout(10000);
}
...
void loop()
{
      ...
      PWM_Activation();
      //Compenser le ration timer en fction du nombre de voie activé
      ...
      {  
          ...
          else
          {
                switch (rec_data)
                {
                    ...
                    case 'T'://Action on IC2
                    {
                          ...
                          else if(a=='F')//F change
                          {
                              ...
                              Serial.flush(); while(Serial.available()==0){ }  Serial.readBytes(&dix, 1);
                              Serial.flush(); while(Serial.available()==0){ }  Serial.readBytes(&unit, 1);
                              f_IC2=10000*translater(dixmil)+1000*translater(mil)+100*translater(cent)+10*translater(dix)+translater(unit);
                          }
                          else if(a=='H') //Pulses number modif
                          {
                              uint8_t mil='N',cent='N', dix='N', unit='N';
                    ...
                    case '_'://Mode Synchro
                    {
                         ...
                         delayMicroseconds(10000);//Timer1.start();
                         break;
                    }
               ...
               if(cpt_cannal==1){ratio_timer=4.23;}
               else if(cpt_cannal==2){ratio_timer=5.837;}
               else if(cpt_cannal==3){ratio_timer=7.4;}
               else if(cpt_cannal==4){ratio_timer=9.205;}
               update_PWM_parameters();
          }              
     }
}
...
//  TIMER_ClearEvent(&TIMER_3);
  ...
  {
    event_count_PWM_IC1++; event_count_PWM_IC2++; event_count_PWM_IC3++; event_count_PWM_IC4++;
    if(PWM_IC1==true)
    {
      //PWM management
      ...
      else if (event_count_PWM_IC1 > (toff_IC1+ton_IC1)){if(nb_pulse_temp_IC1>0 && Pulse_IC1==true){nb_pulse_temp_IC1--;} event_count_PWM_IC1=0;digitalWrite(12,0);}
      //End condition
    ...
    {
        event_count_PWM_IC1=0;event_count_PWM_IC2=0;event_count_PWM_IC3=0;event_count_PWM_IC4=0; // Timer1.stop();
    }
    else {} //Timer1.start();
  }
  else //Part Standard
  {
    ...
    if(PWM_ON==false && Pulse_ON==false){event_count_PWM=0; } //Timer1.stop();} 
    else {} //Timer1.start();
  }
}
...
{
    float ton_base = 1650*dutycycle/frequency; float toff_base=1650*(100-dutycycle)/frequency;
